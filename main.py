    ...
    for uname, ucfg in UNIVERSES.items():
        ...
        if u_state['holdings']:
            ...
            if exits:
                              ...
                              for ex in exits:
                                                    msgs.append(
                                                                              f"  SELL {ex['symbol']} | Price: {ex['price']:.2f} | PnL: {ex['pnl']:+.2f}%"
                                                    )
        ...
        if do_rebal:
            ...
            if signals:
                ...
                msgs.append("No qualifying stocks found. Staying in cash.")
...
def webhook():
    ...
    if text.startswith('/start'):
              tg_answer(chat_id,
                                    "[START] RS+RSI Bot is live on Render!\n\n"
                                    "Commands:\n"
        ...
        if _last_run_time:
            ...
            tg_answer(chat_id, "No scan has run yet. Use /run to trigger one.")
        ...
        tg_answer(chat_id,
                              ...
                              "/help - This message\n\n"
                              f"Params: RS={RS_P} | RSI({RSI_L})<{RSI_T} | SMA={SMA_L} | Top{TOP_N} | Rebal every {REBAL_N}d"
                 )
